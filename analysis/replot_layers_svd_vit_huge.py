...
def plot_results(stats, output_dir):
    ...
    # Set x-axis ticks to show every 2 layers to reduce crowding
    tick_positions = layers[::2]  # Every 2 layers (0, 2, 4, ...)
    ax.set_xticks(tick_positions)
    ax.set_xticklabels([str(i) for i in tick_positions])

    # Add annotations for first, middle, and last layers
    middle_idx = 15  # Middle layer for 32-layer model (0-31)
...
def main():
    ...
    results_path = Path("Output/vit_huge_patch14_224_mae/svd_layers/layer_wise_svd_results.npz")
    output_dir = Path("Output/vit_huge_patch14_224_mae/svd_layers")
