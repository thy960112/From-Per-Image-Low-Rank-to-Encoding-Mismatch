...
def analyze_svd_last_stage(feature_dir, use_gpu=False):
    """Analyze SVD on the last stage of Swin Transformer."""
    ...
    for file_path in tqdm(feature_files, desc="Processing feature maps"):
        # Load feature map (object array for Swin with 4 stages)
        all_stages = np.load(file_path, allow_pickle=True)

        # Extract ONLY the last stage (index -1 or 3)
        # For Swin-Small: Stage 3 has shape (49, 768) - 7x7 tokens, 768 feature dimensions
        X = all_stages[-1]

        # Perform SVD on the last stage feature map
...
def create_plots(results, output_dir):
    """Create all SVD plots for Swin last stage."""
    output_dir = Path(output_dir)
    ...
    plot_single_histogram(
        ranks,
        'Rank Distribution of Feature Maps\n(Swin Last Stage - Stage 3)',
        'Rank',
        'crimson',
...
def main():
    parser = argparse.ArgumentParser(
        description="Analyze SVD on last stage of Swin Transformer features",
    ...
    print("=" * 70)
    print("SVD Analysis on Swin Transformer Last Stage")
    print("=" * 70)
    ...
    # Run analysis
    results = analyze_svd_last_stage(args.feature_dir, use_gpu=args.use_gpu)
